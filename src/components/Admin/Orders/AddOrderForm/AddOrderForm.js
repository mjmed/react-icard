...
export function AddOrderForm(props) {
    ...
    return (
        <Form className='add-order-form' onSubmit={formik.handleSubmit}>
            ...
            <div className='add-order-form__list'>
                { map(productsData, (product, index) => (
                    <div className='add-order-form__list-product' key={index}>
                        ...
                        <Button type='button' icon color='red' onClick={() => removeProductList(index)}>
                            <Icon name='trash' />
                        </Button>
                    </div>
                ))}
            ...
            <Button type='submit' primary fluid disabled={(productsData.length > 0)  ? false : true}>Añadir productos a la mesa</Button>
        </Form>
    )
}
...
function validationSchema() {
    ...
    return {
        products: Yup.array().required('Los productos son obligatorios'),
    }
}